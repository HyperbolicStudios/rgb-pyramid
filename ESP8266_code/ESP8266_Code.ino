...
//For i2c connection with Arduino Nano
#define SDA_PIN 4
#define SCL_PIN 5
...
void setStripe() {
    ...
    //Send RGB via I2C in the format X-XXX (on/off-RGB)
    ...
    Wire.beginTransmission(I2C_SLAVE); // transmit to device #8
    Wire.write(buf);        // sends bytes
    Wire.endTransmission();    // stop transmitting

    Serial.printf("Brightness: %d R: %d G: %d B: %d\n", device_state.brightness, rValue, gValue, bValue);
...
void setup() {
    ...
    Serial.println("Started I2C connection");
